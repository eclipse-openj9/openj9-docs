...
- When using `-Xgcpolicy:gencon` with `-Xsoftmx`, then the proportion of heap space used for nursery within the `-Xsoftmx` limit, will be proportional to the maximum amount of nursery space specified (see [Xmn/Xmnx](xmn.md)) relative to the `-Xmx` value. For example, if the following was specified on the command line `-Xsoftmx2g -Xmnx4g  -Xmx8g`, nursery space will be allowed to use 50%(4G/8G) of the specified `-Xsoftmx` value, which in this example, would be 1G.
- When using `-Xgcpolicy:balanced` with `-Xsoftmx` and `-Xmn`/`-Xmnx`/`-Xmns` options,  the maximum and minimum size for eden are absolute (rather than the proportional nursery behaviour for gencon), and do not depend on `-Xsoftmx` value specified. For example, if `-Xmnx1G` is specified, then eden will be able to grow up to `1G` in size, regardless of the `-Xsoftmx` value specified.
