...
|Stack size for OS Threads **(32-bit)**. Use `-Xmso<size>`           | 256 KB    | 256 KB    | N/A              |  256 KB            |256 KB    |:fontawesome-solid-check:{: .yes aria-hidden="true"}<span class="sr-only">yes</span>| :fontawesome-solid-xmark:{: .no aria-hidden="true"}<span class="sr-only">no</span> |
