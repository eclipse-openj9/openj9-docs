...
| [`-Xss`](xss.md)                                                 | Sets the Java&trade; thread stack size. (Equivalent to `-XX:ThreadStackSize`). <i class="fa fa-pencil-square-o" aria-hidden="true"></i> **Note:** Unlike HotSpot, this option applies only to Java threads. OpenJ9 has a separate native stack for operating system threads (see [`-Xmso`](xmso.md))                                                                      |
