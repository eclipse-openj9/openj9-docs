...
The [`balanced`](gc.md#balanced-policy) policy (`-Xgcpolicy:balanced`) uses two types of cycle to perform GC – a partial GC cycle and a global GC *mark* cycle. The policy GC may have to peform a third type of cycle - a global cycle - to reclaim memory after an allocation failure that has resulted from tight memory conditions.
...
|global cycle|     ???    | `af-start`, `<af-end>`           |An allocation failure. Occurs under tight memory conditions. After multiple cycles the operations of the partial and global *mark* cycles are unable to reclaim enough memory to prevent the whole heap becoming full. A global cycle is needed to free up some memory. Cycle runs very rarely - only occurs under very tight memory conditions. |
...
If the partial GC cycle is run within a global *mark* cycle, the allocation taxation threshold is set to be smaller than the size of the *eden* region to allow for global mark phase increments to run in between partial GC cycles TRUE? Specifically, when the previous partial GC cycle completes, the allocation taxation threshold is set to be half the size of the eden area. The increment, which could be a partial GC cycle or could be a GMP increment, runs.......UNDERSTAND THIS - AM I CORRECT?
...
The `balanced` policy’s global mark GC cycle, which runs when an allocation taxation threshold is reached, consists of a mixture of STW and concurrent operations. The allocation taxation threshold is set to 'tax' application threads to do some work for the GC, such as marking work, for the GC, in 'exchange' for the GC offering more memory for the application threads to allocate for running applications.  

The cycle is split into multiple increments that each run a *global mark phase* and a *global mark phase work packet* sub-increment on the heap. Each *global mark phase* consists of a global *mark* operation that runs during a STW pause, followed by a processing operation that may run concurrently or may use an additional STW pause.
...
```
The first increment, a STW increment, is recorded using the `<gc-start>` element as a `global mark phase`.
...
```
The `global mark phase` increment runs an operation named `mark increment`. This operation begins the process of building a record of object liveness across the heap. 
...
The STW increment ends, as recorded by the `<gc-end>` element. The effect of this first increment on the heap can be determined by comparing the `<mem-info>` attributes and child element attributes at the beginning and end of the increment. Specifically, the marking can be seen by the increase in the `count` value of the `<remembered-set>`.
...
The beginning of the second part of the global *mark* phase increment is recorded. The `GMP work packet processing` operations, recorded by the `<concurrent-start>` element, are run concurrently. The child element `<concurrent-mark-start>` records the scan target of this increment to be over 113GB.
...
Now that the global mark phase sub-increment is complete, application threads are restarted, with the GMP work packet processing sub-increment continuing to run concurrently:

```xml
<exclusive-end id="354" timestamp="2020-11-13T06:32:27.399" durationms="52.216" />
```
The concurrent `GMP work packet processing` sub-increment operations end, recorded using the `<concurrent-end>` element. The child element `<concurrent-mark-end>` shows that the processing OR GMP? has exceeded the scan target of `113.5GB`.
...
The garbage collector now returns to running partial cycles to reclaim free regions in the heap before the next global mark phase increment is triggered. To see an example of how a `balanced` partial GC cycle appears in the logs, see the [`Balanced` examples - Partial GC Cycle](vgclog_examples#partial-gc-cycle).

Following some partial GC cycles, another global mark phase is recorded in the logs. This global mark phase increment is associated with our example global *mark* cycle, having a `contextid` value of `347` that matches the `id=347` value of the cycle.

The increment begins with a STW pause. Details of the memory allocation at the beginning and end of the first part of the increment are automatically recorded using `<gc-start> and ``<gc-end>`elements. The second part of the increment, the `GMP work packet processing` is recorded, again using the `<concurrent-start>` and <concurrent-end>` elements.
...
The next section of the logs shows another `global mark increment `consisting of `global mark phase` sub-increment followed by a `GMP work packet processing` sub-increment. For this increment, the status of the memory before and after the `<gc-start>` and `<gc-end>` STW sub-increment is identical. For some `global mark increments`, this STW sub-increment includes operations such as ADD
...
```
WHERE IS THE `<concurrent-end>` tag?!!!! ADD FROM LOG!!!

More partial cycles run. This pattern of interleaving of intermediate global mark increments with partial gc cycles repeats until the a final global mark increment completes the global mark cycle. The final global mark increment of a global *mark* cycle consists of a STW `global mark phase` sub-increment that includes `mark increment` and `class unload` operations. 
...
The operations to create a record of object liveness across the heap, which together with the partial GC cycle enables the `balanced` GC to reclaim memory, is completed with sweep phase.  While the global *sweep* operation is logically associated with the global *mark* operation, it does not run in the same global *mark* cycle. Instead, the *sweep* operation runs in the same STW increment as the first partial GC cycle that runs after the completion of the global *mark* phase cycle. 

After the log records the end of the global mark cycle it records a STW pause followed by a `partial gc` cycle of `id=501`. The global *sweep* operation that runs after the the global *mark* phase is recorded in the `<gc-op>` element tagged as `id=504`. 
